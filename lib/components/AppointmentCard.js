...
export const AppointmentCard = (appointment) => {
  ...
  return `<div id="${appointment.id}" data-id="${
    appointment.id
  }" class="card bg-white text-neutral rounded-lg p-1 shadow-lg w-full flex-row h-38">
    ...
    "Email"
                    ...
                    <button data-id="${appointment.id}" id="reschedule" class="btn btn-primary h-2/4 text-white btn-sm"><span class="pointer-events-none fill-white w-4">${svgIcons(
                      "Reschedule"
                    )}</span></button>
                  </section>
                </div>
